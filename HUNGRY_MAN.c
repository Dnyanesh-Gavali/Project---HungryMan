    ...
    while (running) {
        ...
        // key controller
            ...
            if (_kbhit()) {
                key = _getch();
                if (key == 'w') {
                    { player.dy = -1; player.dx = 0; player.speed = 1; } // Vertical is slower
                }
                if (key == 's') {
                    { player.dy =  1; player.dx = 0; player.speed = 1; }
                }
                if (key == 'a') {
                    { player.dx = -1; player.dy = 0; player.speed = 2; } // Horizontal is faster   
                }
                if (key == 'd') {
                    { player.dx =  1; player.dy = 0; player.speed = 2; }
                }
                if (key == 'q') {
                    running = 0;
                }
            ...
            for (int i = 0; i < player.speed; i++) {
                ...
                // food eating system
                if (get_distance(player.x, player.y, food.x, food.y) <= 1) {
                        eat = 1;
                        score += 100;
                }
